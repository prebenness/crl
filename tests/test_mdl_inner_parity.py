...
from src.training.train_state import (
    create_state_mdl, create_state_mdl_shared, create_state_outer,
)
...
def test_mdl_inner_step_matches_pair_when_hsic_is_zero():
    ...
    cfg = load_config("config/colored_mnist/ex3_mdl_pair_sweep_light.yaml")
...
def test_mdl_shared_inner_step_matches_pair_when_hsic_is_zero():
    """Shared-MDL inner update should match exactly when HSIC is disabled."""
    cfg = load_config("config/colored_mnist/ex3_mdl_pair_sweep_light.yaml")
    cfg.training.batch_size = 16
    cfg.mdl.n_samples = 3
    cfg.mdl.shared_lambda2 = 20.0
    cfg.mdl.shared_epsilon = 1e-6
    ...
    init_key = jrandom.PRNGKey(31)
    inner_single = create_state_mdl_shared(
        init_key, inner_model, (cfg.training.batch_size, 28, 28, 3), cfg,
    )
    inner_pair = create_state_mdl_shared(
        init_key, inner_model, (cfg.training.batch_size, 28, 28, 3), cfg,
    )
    outer_state = create_state_outer(
        jrandom.PRNGKey(32), outer_model, (cfg.training.batch_size, 28, 28, 3), cfg,
    )

    x = jrandom.uniform(
        jrandom.PRNGKey(41), (cfg.training.batch_size, 28, 28, 3),
    )
    y = jrandom.randint(
        jrandom.PRNGKey(42), (cfg.training.batch_size,), 0, cfg.model.num_classes,
    )
    step_rng = jrandom.PRNGKey(43)
    mdl_lambda = 1e-1
    n_train = 60_000
